...
namespace LegTracking.FootTracking
{
    public class FootTracker : MonoBehaviour
    {
        ...
        [SerializeField] private float rootOffsetX = 0f;
        [SerializeField] private float rootOffsetY = 0f;
        ...
        private void Start()
        {
            Invoke(nameof(SetupFoot), 1f);
        }

        private void SetupFoot()
        {
            if (trackerTransform == null || footTransform == null)
            {
                Debug.LogError("FootTracker: trackerTransform veya footTransform boş.");
                return;
            }
        ...
        private void LateUpdate()
        {
            ...
            var offset =
                forward * rootOffsetZ +
                right * rootOffsetX +
                Vector3.up * rootOffsetY;

            footTransform.position = trackerTransform.TransformPoint(_footLocalPosition) + offset;
        }
    }
}